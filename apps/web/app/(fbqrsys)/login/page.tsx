...
/**
 * FBQRSYS login page — email + password for SystemAdmin accounts.
 * On success, redirects to /fbqrsys/dashboard (or callbackUrl if present).
 * Middleware handles mustChangePassword redirect after login.
 */
import { useState, type FormEvent } from "react";
import { signIn } from "next-auth/react";
import { useRouter, useSearchParams } from "next/navigation";

export default function FbqrsysLoginPage() {
  const router = useRouter();
  const searchParams = useSearchParams();
  const callbackUrl = searchParams.get("callbackUrl") ?? "/fbqrsys/dashboard";
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    const result = await signIn("fbqrsys", {
      email,
      password,
      redirect: false,
    });
