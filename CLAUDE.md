...
- If order expired **less than** the window ago → run revival checks (all must pass):
  ...
  - If all checks pass → **revive**: `Order.status → CONFIRMED`, order pushed to kitchen. Rationale: the customer paid real money.
  - If any check fails → **auto-refund** via Midtrans Refund API + notify merchant.
